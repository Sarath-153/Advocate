...
export default function Contact() {
  ...
  const inputClass =
    "w-full px-3 md:px-4 py-3 rounded-xl border border-gray-200 text-sm text-[#1a1f3c] placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-[#1a1f3c] transition";
  const labelClass = "text-xs font-semibold text-gray-400 uppercase tracking-wide";

  const contactItems = [
    {
      label: "Phone",
      value: "+91 95005 73949",
      href: "tel:+919500573949",
      icon: "M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z",
    },
    {
      label: "Email",
      value: "sriarjunac@gmail.com",
      href: "mailto:sriarjunac@gmail.com",
      icon: "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z",
    },
    {
      label: "Head Office",
      value: "No. D-88 Sampath Nagar, Erode - 638 011",
      icon: "M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0zM15 11a3 3 0 11-6 0 3 3 0 016 0z",
    },
    {
      label: "Branch Offices",
      value: "Coimbatore | Chennai | Madurai | New Delhi | Erode",
      icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4",
    },
  ];

  // ─── Reusable JSX blocks (written ONCE, used in both mobile & desktop) ───
      ...
      <div className="flex flex-col gap-3">
        {contactItems.map(({ label, value, href, icon }) => {
          ...
          return (
            ...
            >
              <div className="w-10 h-10 md:w-11 md:h-11 rounded-xl bg-[#1a1f3c] flex items-center justify-center shrink-0">
        ...
        <div className="space-y-2 text-sm">
          {[
            ...
            <div key={day} className="flex justify-between">
              ...
              <span className={`font-semibold ${closed ? "text-red-400" : "text-[#1a1f3c]"}`}>{hours}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  const FormBlock = (
    <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-5 md:p-8">
      {submitted ? (
        ...
        <>
          <h3 className="text-base md:text-lg font-bold text-[#1a1f3c] mb-5 md:mb-6">Send us a message</h3>
    ...
    <section id="contact" ref={ref} className="bg-[#f8f9fc] py-12 md:py-20 px-5 md:px-20 scroll-mt-20">
      ...
      <div className="md:hidden mb-6">
        <div className="flex bg-white rounded-2xl p-1 shadow-sm border border-gray-100">
          {(["form", "info"] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200
                ${activeTab === tab ? "bg-[#1a1f3c] text-white shadow" : "text-gray-400"}`}
            >
              {tab === "form" ? "Send Message" : "Our Info"}
            </button>
          ))}
        </div>
      </div>

      {/* MOBILE: Show active tab content only — hidden on desktop */}
      <div className="md:hidden">
        {activeTab === "form" ? FormBlock : InfoBlock}
      </div>
