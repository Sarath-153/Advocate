...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        {/* LOGO */}
        <h1 className="text-xl font-bold cursor-pointer tracking-widest text-gray-800"
          onClick={() => scrollToSection("home")}>LOGO</h1>
          ...
          {links.map((link) => (
            <button key={link.id} onClick={() => scrollToSection(link.id)}
              className={`px-5 py-2 rounded-full transition-all duration-200 text-sm
                ${link.id === "home" ? "bg-[#1a1f3c] text-white" : "hover:text-gray-900"}`}>
              {link.label}
            </button>
          ))}
        </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            {links.map((link) => (
              <button key={link.id} onClick={() => scrollToSection(link.id)}
                className="text-left text-[#1a1f3c] font-medium text-lg border-b border-gray-100 pb-3 hover:text-blue-600 transition-colors">
                {link.label}
              </button>
            ))}
          </motion.div>
